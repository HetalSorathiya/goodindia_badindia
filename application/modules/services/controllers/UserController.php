...
class Services_UserController extends GTL_Action {
        ...
        if ($this->getRequest()->isPost()) {
           ...
           /* if ($postedData['lgn_email'] == '') {
			...
			if ($postedData['usr_dob'] == '') {
                $errorMessageArray[] = 'Please enter Date Of Birth.';
            }
            ...
            if (count($errorMessageArray) > 0) {
				...
				$userData['usr_dob'] = $postedData['usr_dob'];
                ...
                $JSONARRAY = Array(
                    ...
                    'DATA' => $resultArray,
						...
						$emailParuser = array(
							'fileName' => 'register_user.phtml',
							'subjectName' => 'User Registration Complete',
							'FromEmailId' => $ADMIN_MAIL,
							'FromName' => $ADMIN_FROM,
							'toEmailId' => 'hetal.citrusbug@gmail.com',
							'toName' => $ADMIN_FROM,
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!$LikepostForm->isValid($postedData)) {
				...
				$totallikepost = $likepostModel->fetchPostLike($wherelike);
					...
					if(count($totallikepost)>0){
						...
						/* save data the notification log table*/
								...
								if(count($errorMessageArray)>0){
									...
									$postcommentData['notf_cmt_lgn_id'] = $postedData['like_lgn_id'];
									$postcommentData['notf_cmt_post_id'] = $postedData['like_post_id'];
									$postcommentData['notf_createddate'] = date('Y-m-d H:i:s');
									if($postedData['like_type']=='1')
									{
									$postcommentData['notf_status'] = '2';
									}else
									{
										$postcommentData['notf_status'] = '3';
									}
					...
					$deviceid='c0cH1LCSE3I:APA91bGmYsTcsXcon6hG6uco0K1Lm51YjoD9pdoKYEmW5xaPlupY9ghPprR--QHSgADpCzKH5y8kkP1wirBB-NFwY8nljc3RSWBzHpL9SlWecc5VTJKIi8G8blGjdCZMOpi-NOItfTST';
	...
	public function addcommentAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!$AddcommentForm->isValid($postedData)) {
				...
				$edit_image = $userExist['usr_update_img'];
				$social_image = $userExist['usr_image'];				
			 ...
			 if (count($errorMessageArray) > 0) {
				...
				foreach($PostcommentArray as $p_key => $p_val){
					//echo"<pre>";print_r($p_key);exit;
						...
						/* fetch User Profile */
						$gender=$p_val['usr_gender'];
						$edit_image = $p_val['usr_update_img'];
						$social_image = $p_val['usr_image'];				
						$fetchuserprofileimg = $userModel->fetchuserprofileimg($social_image,$edit_image,$gender);
				
					
					$finalArray[$p_key] = $p_val;
					//$finalArray[$p_key]['reply'] = array_values($Postrefcomment[$p_val['cmt_id']]);
					$finalArray[$p_key]['usr_image'] = $fetchuserprofileimg;
    ...
    public function commentreplylistAction() {
		...
		$postModel = new Model_Webservice_Post(); // Load post Model
		$userModel = new Model_Webservice_User(); // Load post Model
        $postcommentModel = new Model_Webservice_Postcomment(); // Load post Model
		$likecommentModel = new Model_Webservice_Commentlike(); // Load post like model
		if ($this->getRequest()->isPost()) {
            $postedData = $this->_request->getPost();
	 ...
	 public function userprofileAction() {
		...
		if ($this->getRequest()->isPost()) {
			...
			if (count($errorMessageArray) > 0) {
				...
				$social_image = $UserArray['usr_image'];
				$edit_image = $UserArray['usr_update_img'];
				...
				}else if($gender==0 && $usrupdateimg =='' && $usriimage==''){
				...
				$fetchuserprofileimg = $userModel->fetchuserprofileimg($social_image,$edit_image,$gender);
				$UserArray['usr_image']=$fetchuserprofileimg;
			//echo"<pre>";print_r($fetchuserprofileimg);exit;
			...
			if ($postedData['usr_lgn_id'] != "") {
			   $userExist = $loginModel->fetchEntryById($postedData['usr_lgn_id']);		   
                ...
                if (count($errorMessageArray) > 0) {
					...
					$userProfileData['usr_id'] = $userExist['usr_id'];
                    $userProfileData['usr_dob'] = $postedData['usr_dob'];
					$userProfileData['usr_gender'] = $postedData['usr_gender'];
...
//echo"<pre>";print_r($postExist);exit;				   
					if (count($postExist) == 0) {
						$errorMessageArray[] = "Post doesn't exist.";
					}
				}
