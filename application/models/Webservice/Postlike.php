...
class Model_Webservice_Postlike extends Model_Base {
	...
	/**
     *  This function is used to get Post Like
     * @return null|Zend_Db_Table_Row_Abstract
     */
    ...
    /**
     *  This function is used to get total Post Like
     * @return int
     */
	...
	public function fetchPostLike($where) {
        $select = $this->select()
				->setIntegrityCheck(false)
                ->from(array("postgood" => $this->POST_LIKE_TBL))
	...
	/* public function UpdatePostLike($where) {
        $select = $this->select()
				->setIntegrityCheck(false)
                ->from(array("postgood" => $this->POST_LIKE_TBL));
				//->joinleft(array('usr' => $this->USER_TBL), "usr.usr_lgn_id = postgood.like_lgn_id")
